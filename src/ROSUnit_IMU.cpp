...
ROSUnit_IMU::ROSUnit_IMU(ros::NodeHandle& t_main_handler) : ROSUnit(t_main_handler) {
    ...
    _output_port_5 = new OutputPort(ports_id::OP_5_FREE_ACCELERATION, this);
    _ports = {_output_port_0, _output_port_1, _output_port_2, _output_port_3, _output_port_4, _output_port_5};
}
...
void ROSUnit_IMU::callbackXsensFreeAcceleration(const geometry_msgs::Vector3Stamped& msg_free_acceleration){
    ...
    _instance_ptr->_output_port_5->receiveMsgData(&pv_dot_dot_msg);
}
...
//TODO move to outside ROSUnit
    ...
    orientation_euler.x = 1 * _euler.x;
    orientation_euler.y = 1 * _euler.y;
